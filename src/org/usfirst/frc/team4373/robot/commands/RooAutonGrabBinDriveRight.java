...
public class RooAutonGrabBinDriveRight extends RooAutonBase {
	...
	public void execute() {
		oi.iterate();
		if (shouldRaise) {
			if (Robot.rooForkLift.moveToPosition(savedthang+ 4)) {
...
//				oi.rd.putNumber(", value);
		...
		if (hasRaisedArm & !(hasLoweredArm)){
			Robot.rooDrivetrain.setStrafe(oi.rd.rooGetNumber("Auton Strafe Power", 0.5));
			try{
				Thread.sleep((long)oi.rd.rooGetNumber("Auton strafe drive delay: ", 4000)); //make this shorter to automove to the inbound station
